    ...
    <div className="max-w-2xl mx-auto px-4 py-16">
        ...
        <div className="bg-white dark:bg-[#0d1424] border border-slate-200 dark:border-[#1e2d45] rounded-2xl p-6">
          ...
          <form onSubmit={handleSearch} className="flex gap-2">
            ...
            <button
              type="submit"
              disabled={searching}
              className="px-5 py-2.5 bg-gradient-to-r from-blue-600 to-violet-600 text-white text-sm font-semibold rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50"
            >
              {searching ? '…' : 'Search'}
            </button>
          </form>
          ...
          {results.length === 0 && query && !searching && (
            <p className="mt-4 text-sm text-slate-400 text-center">No members found. Try a different name.</p>
          )}
        </div>
      )}
      ...
      {selected && (
        <div>
          ...
          <div className="bg-white dark:bg-[#0d1424] border border-slate-200 dark:border-[#1e2d45] rounded-2xl p-6 mb-4">
            <div className="flex items-center gap-3 mb-1">
              {selected.avatarUrl && <img src={selected.avatarUrl} alt={selected.name} className="w-10 h-10 rounded-full object-cover" />}
              <div>
